...
export function FolderBlock({
  ...
  return (
    <div className="overflow-hidden rounded-card border border-line">
      <div className="flex items-center justify-between gap-2 bg-surface-sunken px-4 py-2">
        {renaming ? (
          <form
          ...
          <>
            <button type="button" onClick={() => setRenaming(true)} className="flex min-h-6 items-center gap-2 text-sm font-medium text-ink-2">
              ...
              {folder.nome}
              <span className="font-normal text-ink-3">({campaigns.length})</span>
            </button>
            <form
      ...
      {campaigns.length === 0 ? (
        <p className="px-4 py-3 text-xs text-ink-3">Nenhuma campanha nessa pasta ainda.</p>
      ) : (
        campaigns.map((c, i) => (
          <CampaignRow
            key={c.id}
