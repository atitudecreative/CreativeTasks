...
import { updateMyTheme, resetMyTheme } from "./actions";
...
function SubmitButton() {
  ...
  return (
    ...
    >
      {pending ? "Salvando..." : "Salvar minhas cores"}
    </button>
  );
}
...
export function ThemeSettingsForm({
  ...
  const [state, formAction] = useFormState(updateMyTheme, { error: null as string | null });
  ...
  return (
    <div className="space-y-6">
      <form action={formAction} className="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm">
        <p className="mb-4 text-sm font-medium text-neutral-700">Minhas cores</p>
        ...
        <div className="flex items-center gap-3">
          ...
          {hasCustom && (
            <button
              type="button"
              onClick={() => {
                if (window.confirm("Voltar a usar a cor padrão do site?")) {
